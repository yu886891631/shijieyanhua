...
如果已经有工作流助手，只想把当前工作流改成六阶段而保留原脚本的 API 等设置，请打开工作流助手设置，在“工作流预设”一栏点击导入按钮，导入：

- `导入到酒馆中/工作流预设-世界后台引擎-六阶段轻量版.json`
...
六阶段工作流依次为：世界状态 → 名单筛选与 ReplicaEnum → 前台/后台角色副本 → 后台交互 → 社交圈变量 → 资产账本。资产账本默认关闭，因此通常运行五个阶段。
