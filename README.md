...
如需 NPC 在主角视线之外继续行动，可额外导入：

- `导入到酒馆中/酒馆助手脚本-世界演变-GitHub版.json`

该插件独立于六阶段工作流，默认关闭；启用后可在工作流助手完成并至少有一个任务成功时自动触发，也可从“打开世界演变面板”手动运行。它读取本轮消息与 MVU 快照，将后台状态单独保存在浏览器 IndexedDB，不写回 MVU 或 shujuku。世界书同步需要在面板中填入一个已经存在的世界书名称。

这是用于 SillyTavern 实测的第一版原型，运行数据按聊天保存在当前浏览器中。清理酒馆站点数据或更换浏览器前，请先从面板导出备份。详细设计和当前边界见 `docs/WORLD-EVOLUTION-PLAN.md`。

## 开发中：世界演变数据库 A0.1.0

数据库版目前与旧版面板兼容，真实世界演变结果已经以独立数据库为唯一事实源：

- 源码：`src/世界演变数据库/`
- 设计：`docs/WORLD-EVOLUTION-DB-DESIGN.md`
- M3 导入文件：`导入到酒馆中/酒馆助手脚本-世界演变数据库-A0.1.0-alpha.3.json`
- M3 世界演变插件：`导入到酒馆中/酒馆助手脚本-世界演变-GitHub版-A0.1.0-alpha.3.json`
- M4 数据库导入文件：`导入到酒馆中/酒馆助手脚本-世界演变数据库-A0.1.0-alpha.4.json`
- M4 世界演变插件：`导入到酒馆中/酒馆助手脚本-世界演变-GitHub版-A0.1.0-alpha.4.json`
- M5 数据库导入文件：`导入到酒馆中/酒馆助手脚本-世界演变数据库-A0.1.0-alpha.5.json`
- M5 世界演变插件：`导入到酒馆中/酒馆助手脚本-世界演变-GitHub版-A0.1.0-alpha.5.json`
- M6 数据库导入文件：`导入到酒馆中/酒馆助手脚本-世界演变数据库-A0.1.0-alpha.6.json`
- M6 世界演变插件：`导入到酒馆中/酒馆助手脚本-世界演变-GitHub版-A0.1.0-alpha.6.json`
- M8 数据库导入文件：`导入到酒馆中/酒馆助手脚本-世界演变数据库-A0.1.0-alpha.8.json`
- M8 世界演变插件：`导入到酒馆中/酒馆助手脚本-世界演变-GitHub版-A0.1.0-alpha.8.json`

alpha.8 在 alpha.7 的管理面板基础上加入 M8 A0.0.5 迁移预检、冲突报告、原始备份保留、事务回滚以及迁移后当前角色卡 `primary` 世界书投影重建。世界书仍然只是可重建投影，数据库才是事实源。所有相关测试均使用固定模拟输入，不调用真实 API。测试阶段导入脚本暂指向 `@main`，正式发布时再切换到不可变 tag。
